...
# cityscapes imports
sys.path.append( os.path.normpath( os.path.join( os.path.dirname( __file__ ) , '..' , 'helper' ) ) )
...
def createLabelImage(annotation, encoding, outline=None):
    ...
    # the background
    if encoding == "ids":
        background = name2label['unlabelled'].id
    elif encoding == "trainIds":
        background = name2label['unlabelled'].trainId
    elif encoding == "color":
        background = name2label['unlabelled'].color
    else:
